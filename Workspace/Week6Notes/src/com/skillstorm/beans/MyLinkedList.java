...
public class MyLinkedList<T> { // <> tell Java that this is a Generic
	...
	private Node head; // first element in the LinkedList
	private Node tail; // need this so I don't need to traverse everytime I add something to the end
	private int size;
	...
	public T get(int index) {
		...
		if (index >= size || index < 0) {
			// if the index is greater than the size, it doesn't exist
	...
	public void insert(int index, T value) {
		if (index == size) {
			// really just inserting something at the end, exactly like my add method does
			add(value);
			...
			if (index == 0) {
				...
				// if I'm not adding it to the beginning
	...
	public void print() {
		...
		while(current != null) {
			// null is the end of my LinkedList
