...
public class JavaFoundations {
	...
	public static void multiDimensionalArrays() {
		...
		// I can manually define different sized at a later point
		...
		// can use a foreach loop same with other arrays
		// loop through each element in our 3 x 4 array nums
		System.out.println("\n***** Foreach Nums *****");
		for(int[] row : nums) {
	...
	public static void sets() {
		...
		// Iterator, special class that represents an unchanging set of the values
		// that I can then loop over
		Iterator<String> hasSetIterator = names.iterator();
		...
		while(hasSetIterator.hasNext()) {
			...
			String name = hasSetIterator.next(); 
			System.out.println(name);
		}
	...
	public static void maps() {
		...
		// Map is an interface just like List, Queue, Dequeue (Stack)
		...
		// much like a list's get, where you search off the index.
		// You can search a hashMap by key
		...
		// put overwrite what is there if the key already exists, else it adds it
		...
		// HashMap uses the hash, TreeMap sorts based on a Comparable
		...
		System.out.println("\n****** Inventory with Comparable *****");
		// HashMap are unsorted while TreeMpas are sorted. Either based on a comparable
		// or a comparator
	...
	public static void moreDataStructures() {
		...
		// if i don't want to print the whole thing
		// but still want to see what is on top of my stack
		...
		while(stack.size() != 0) { // or can use (stack.peek() != null)
			int place = stack.size(); // peek does not remove it
	...
	public static void genericClass() {
		// can hand anything to MyGeneric and it works
		...
		// the types don't matter as long as what I assign to a is always the first type
...
//		System.out.println(carAL.get(0));
		...
		// LinkedList work differently than ArrayList
		...
		// can't use these with a List / ArrayList. LinkedList specific
		System.out.println(carLL.peek()); // access the first element
		System.out.println(carLL.getLast()); // access the last element
	...
	public static void staticStuff() {
		...
		car1.drive(); // Vehicle.drive (because of static)
		car2.drive(); // Sedan.drive
