...
public class WeekFour {
	...
	public static void main(String[] args) {
		...
		// don't do this, it will kill your program every time
		// callMe();
	...
	public static void failGracefully() {
		...
		// you wrap risky code inside the try { } and then your error 
		// handling code inside the catch() { } you NEED a catch for every try
		
		// int otherResult = 5 / 0; // only the code inside a try is protected
		...
		try {
			...
			// if the code above didn't work because of an ArrayIndexOutOfBoundException run this
			// nums[2] = 3; // nums doesn't exist here
		...
		try {
			...
			int result = 3 / vals[3]; // ArrayIndexOutOfBoundException	
			...
			System.out.println("Catches everythings");
		}
		...
		try {
			...
			// vals[Integer.parseInt(obj.toString())] = 3 / val;
			// obj.doWheelie(); // NullPointerException
			int result = 3 / vals[3]; // ArrayIndexOutOfBoundException
		} catch(Exception ex) {
		...
		//moreExceptions(); // throws and exception
	...
	public static void randomization() {
		...
		for (int i = 0; i < 10; i++) {
			...
			// high number is not inclusive
			System.out.println(Math.random() * (121 - 115) + 115);
		}
