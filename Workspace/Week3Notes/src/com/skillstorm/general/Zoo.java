...
public class Zoo {
	...
	// usually final variables are in all caps, to signal anyone who uses
	...
	public static void main(String[] args) {
		// inside the same class I do not need a class name for method class
		...
		// can do this because it's default and i'm in the same package
		myAnimal.color = "Grey";
		
		// can do this because it's protected and i'm in the same package
		myAnimal.age = 53;
		
		// cannot access this here because it is private and I am not inside 
		// the animal class
		// myAnimal.fur = true;
	...
	public void polymorphism() {
		...
		System.out.println("****** Bike Comparison ******");
		for (int i = 0; i < myBikes.length; i++) {
			...
			// System.out.println(myBikes[i]instanceof Sport); // another way to check
			...
			for (int j = myBikes.length - 1; j > -1; j--) {
				...
				// == for objects compares the memory location, which isn't always we want
				// .equals is better
		...
		// null is a valid value for non-primitivies (objects), but it is
	...
	public void rideBike() {
		...
		// the data type for myThirdBike is Motorcycle
		myThirdBike.drive();
		myThirdBike.park();
		myThirdBike.honk();
	...
	public void sharkExhibit() {
		...
		try {
			...
			// cannot do that with HammerHead or Goblin because they do not implement 
			// fish
	...
	private void sharkBait(Shark[] sharks) {
		for (int i = 0; i < sharks.length; i++) {
			try {
			sharks[i].swim(320);
			System.out.println("There's a bait in the water. Hoo HaHa!");
			sharks[i].hunt();
			} catch(CloneNotSupportedException ex) {
	...
	public void birdExhibit() {
		...
		// but i didn't had to retype them
		...
		bords[1] = baldilocks; // a bald eagle is a bird
		bords[2] = michael; // a penguin is a bird
		bords[3] = larry; // a bird is a bird
	...
	private void viewBirds(Bird[] birds) {
		...
		// java will look at the underlying type and use that method
