...
public class Sedan extends Vehicle implements Comparable<Sedan>{ // replace T with class name
	
	private String owner = "Dan Pickles";
	
	// you can also use this() keyword to call any of your constructors
	public Sedan() {
	...
	// now if I want to able to order objects (say one is > or < another object
	...
	public int compareTo(Sedan other) {
		...
		// in our case we are probably alphabetizing base off make and model
	...
	// need a nested class for this, don't need, but a bit easier with one
	public class SedanColorComparator implements Comparator<Sedan>{
		// can set up comparisons uses the Comparator interface
		
		// method only really makes sense when the class isn't static
		public void method() {
			// can't use this to access these, but I can access sedan's methods from here
	...
	// static nested class - exactly the same as a non-static one except you don't need a version
