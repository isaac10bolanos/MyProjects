...
public class Program {
	...
	// doesn't need to be final, everyone just needs access to it
	...
	// doesn't help with thread safety
	...
	public static void manageAccount(Person p) {
		try {
			for(int i = 0; i < 3; i++) {
				// for random, it's our random * (high - low) + low
	...
	public static void threadSafety() {
		...
		// any time you change a variable
		// any time we look at a condition that stems from a shared variable
		...
		// if my threads are running the same code I can create them in a loop
		...
		for(int i = 0; i < 2; i++) {
			...
			Thread temp = new Thread(() -> {
				...
				for(int j = 0; j < 10; j++) {
					...
					// at a time can run, which ever thread grabs mutex first
					// when the block exists, the lock is released
					synchronized (mutex) {
		 ...
		 * notify(): returns a single thread to runnable
...
//		}
	...
	public static void threads() {
		...
		// threads don't run / are not fully created until you call start
		// start calls the run method of a runnable
		t1.start();
		t2.start();
		
		// the order i call start in is not the order they run in
		// the order of execution is never guarantee
		...
		// can't remember what number I left off at so we jumped to 6
		 ...
		 * 	timed waiting: the thread has been told to wait for an amount of time
		 * 				   Does not releases any locks
		...
		// Objects, and anything that exists before the thread is not thread safe
		// there are thread safe implementations of certain objects
		
		// reading doesn't change values, so its generally thread safe, any time you
		// change a values is when you have to consider thread safety
		NumHolder num = new NumHolder();
		num.x = 0;
		...
		// but what if I don't want to wrap my primitives in an object
