...
public class ProducerConsumer {
	...
	// going to house the shared resource they are producing to and consuming from
	...
	public void consume(int data) {
		synchronized (mutex) {
			// no guarantee the data it would like to consume is available
			System.out.println(Thread.currentThread() + " wants to consume " + data);
			
			// if the data is not available the thread needs to wait
			// our consumer cannot leave the critical section if it does not
			// consume the data it needs 
			// the code for both producers and consumer still need the lock to continue
